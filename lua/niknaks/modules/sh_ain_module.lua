...
do
	...
	local function scan( node, tab, zone )
		for _, t in pairs( node:GetConnections() ) do
			local n = t[1]
			if tab[n:GetID()] then continue end -- Already added
			tab[n:GetID()] = n
			local z = n:GetZone()
	...
	local function parseMap( self )
		for _, v in pairs( NikNaks.CurrentMap:GetEntities() ) do
			...
			local node = self:FindNode( v.origin, _type )
			if not node then
	...
	local thisMap, thisMapObject = "maps/graphs/" .. game.GetMap() .. ".ain"
...
do
	local function scan( node, tab )
		for _, t in pairs( node:GetConnections() ) do
			if tab[t[1]:GetID()] then continue end -- Already added
			tab[t[1]:GetID()] = t[1]
			scan(t[1], tab)
		end
	end
...
do
	...
	---@param fileName string
	function n_meta:SaveAin( fileName )
		fileName = fileName or self._file or "nodegraph"
		if not string.match(fileName,".dat$") then fileName = fileName .. ".dat" end -- Add file type. We use .dat to stop users from opening it in notepad.
		self:SaveToBuf():SaveToFile( fileName )
		print("[NodeGraph]: Saved to " .. tostring("data/" .. fileName) )
	end
	...
	function n_meta:GenerateZones()
		...
		for i = 1, #nodes do
			...
			zone = zone + 1 -- Increase the zone number
		end
