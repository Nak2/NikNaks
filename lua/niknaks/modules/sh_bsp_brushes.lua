...
function meta:GetBrushes()
	...
	for i = 1, math.min(data:Size() / 96, MAX_MAP_BRUSHES) do
		local t = {}
		...
		for i = first, first + num - 1 do
			t.sides[n] = self:GetBrusheSides()[i]
			n = n + 1
		end
		setmetatable(t, meta_brush)
		self._brushes[i] = t
	end
	self:ClearLump( 18 )
...
function meta_brush:BrushesToTriangles(  )
	local polygons = {}
	for i = 1, self.numsides do
		local side = self.sides[i]
		local plane = side.plane
		local poly = {}
		polygonFromPlanes(arrayOfPlanes, i, poly)
	end
end